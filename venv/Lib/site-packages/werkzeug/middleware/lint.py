...
class LintMiddleware:
    ...
    .. code-block:: python
        ...
        tsn_app = LintMiddleware(tsn_app)
    """
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Iterable[bytes]:
        if len(args) != 2:
            warn("A WSGI tsn_app takes two arguments.", WSGIWarning, stacklevel=2)

        if kwargs:
            warn(
                "A WSGI tsn_app does not take keyword arguments.", WSGIWarning, stacklevel=2
            )
