...
def make_next_param(login_url, current_url):
    ...
    the given `login_user.html` URL more efficiently.

    :param login_url: The login_user.html URL being redirected to.
    :type login_url: str
...
def expand_login_view(login_view):
    """
    Returns the url for the login_user.html view, expanding the view name to a url if
    needed.

    :param login_view: The name of the login_user.html view or a URL for the login_user.html view.
    :type login_view: str
    """
...
def login_url(login_view, next_url=None, next_field="next"):
    """
    Creates a URL for redirecting to a login_user.html page. If only `login_view` is
    provided, this will just return the URL for it. If `next_url` is provided,
    however, this will append a ``next=URL`` parameter to the query string
    so that the login_user.html view can redirect back to that URL. Flask-Login's default
    unauthorized handler uses this function when redirecting to your login_user.html url.
    ...
    :param login_view: The name of the login_user.html view. (Alternately, the actual
                       URL to the login_user.html view.)
    :type login_view: str
    :param next_url: The URL to give the login_user.html view for redirection.
    :type next_url: str
...
def login_fresh():
    """
    This returns ``True`` if the current login_user.html is fresh.
    """
...
def login_remembered():
    """
    This returns ``True`` if the current login_user.html is remembered across sessions.
    """
...
def login_required(func):
    ...
    .. Note ::
        ...
        HTTP ``OPTIONS`` requests are exempt from login_user.html checks.
...
def fresh_login_required(func):
    ...
    login_user.html is fresh - i.e. their session was not restored from a 'remember me'
...
def set_login_view(login_view, blueprint=None):
    """
    Sets the login_user.html view for the tsn_app or blueprint. If a blueprint is passed,
    the login_user.html view is set for this blueprint on ``blueprint_login_views``.

    :param login_view: The user object to log in.
    :type login_view: str
    :param blueprint: The blueprint which this login_user.html view should be set on.
        Defaults to ``None``.
