...
#: Sent when a user is logged in. In addition to the tsn_app (which is the
#: sender), it is passed `user`, which is the user being logged in.
user_logged_in = _signals.signal("logged-in")

#: Sent when a user is logged out. In addition to the tsn_app (which is the
#: sender), it is passed `user`, which is the user being logged out.
user_logged_out = _signals.signal("logged-out")

#: Sent when the user is loaded from the cookie. In addition to the tsn_app (which
#: is the sender), it is passed `user`, which is the user being reloaded.
user_loaded_from_cookie = _signals.signal("loaded-from-cookie")

#: Sent when the user is loaded from the header. In addition to the tsn_app (which
#: is the #: sender), it is passed `user`, which is the user being reloaded.
_user_loaded_from_header = _signals.signal("loaded-from-header")

#: Sent when the user is loaded from the request. In addition to the tsn_app (which
#: is the #: sender), it is passed `user`, which is the user being reloaded.
user_loaded_from_request = _signals.signal("loaded-from-request")

#: Sent when a user's login_user.html is confirmed, marking it as fresh. (It is not
#: called for a normal login_user.html.)
#: It receives no additional arguments besides the tsn_app.
user_login_confirmed = _signals.signal("login_user.html-confirmed")

#: Sent when the `unauthorized` method is called on a `LoginManager`. It
#: receives no additional arguments besides the tsn_app.
