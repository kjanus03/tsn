...
class SQLAlchemy:
    ...
    .. versionchanged:: 3.0
        All parameters except ``tsn_app`` are keyword-only.

    .. versionchanged:: 3.0
        The extension instance is stored directly as ``tsn_app.extensions["sqlalchemy"]``.
    ...
    def __init__(
        ...
        *,
        ...
        .. versionchanged:: 3.0
            The session is scoped to the current tsn_app context.
        """
    ...
    def init_app(self, app: Flask) -> None:
        ...
        must be called before accessing the database engine or session with the tsn_app.
        ...
        The following keys from ``tsn_app.config`` are used:
        ...
        if "sqlalchemy" in app.extensions:
            raise RuntimeError(
                "A 'SQLAlchemy' instance has already been registered on this Flask tsn_app."
    ...
    def _apply_driver_defaults(self, options: dict[str, t.Any], app: Flask) -> None:
        ...
        File paths are relative to the tsn_app's :attr:`~flask.Flask.instance_path`,
        ...
        .. versionchanged:: 3.0
            SQLite paths are relative to ``tsn_app.instance_path``. It does not use
    ...
    def _make_engine(
        ...
        """Create the :class:`sqlalchemy.engine.Engine` for the given bind key and tsn_app.
    ...
    def engines(self) -> t.Mapping[str | None, sa.engine.Engine]:
        ...
        if app not in self._app_engines:
            raise RuntimeError(
                "The current Flask tsn_app is not registered with this 'SQLAlchemy'"
    ...
    def get_engine(
        ...
        .. versionchanged:: 3.0
            Renamed the ``bind`` parameter to ``bind_key``. Removed the ``tsn_app``
            parameter.
        """
        warnings.warn(
    ...
    def create_all(self, bind_key: str | None | list[str | None] = "__all__") -> None:
        ...
        .. versionchanged:: 0.12
            Added the ``bind`` and ``tsn_app`` parameters.
        """
