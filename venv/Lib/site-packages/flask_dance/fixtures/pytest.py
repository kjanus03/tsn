...
``tsn_app`` and ``flask_dance_sessions``. The ``tsn_app`` fixture must
return the Flask tsn_app that is being tested, and the
...
.. code-block:: python
    ...
    from myapp import tsn_app as _app
...
.. code-block:: python
    ...
    def test_home_page(tsn_app):
        with tsn_app.test_client() as client:
