...
def login_user(
    ...
    request (i.e. ``tsn_app.security.datastore.commit()``).
...
def check_and_update_authn_fresh(
    ...
    :param method: Optional - if set and == "basic" then will always return True.
                  (since basic-security sends username/password on every request)
    ...
    Flask security adds this state to the session (cookie) and the security token.
    ...
    .. versionchanged:: 5.5.0
        ...
        which is set from either session or security token
    """
...
def verify_and_update_password(password: str | bytes, user: UserMixin) -> bool:
    ...
    (i.e. ``tsn_app.security.datastore.commit()``).
...
def suppress_form_csrf():
    ...
    If tsn_app doesn't want CSRF for unauth endpoints then check if caller is authenticated
...
def get_url(endpoint_or_url: str, qparams: dict[str, str] | None = None) -> str:
    ...
    try:
        ...
        # This is an external URL (no endpoint defined in tsn_app)
...
def csrf_cookie_handler(response: Response) -> Response:
    ...
    *SESSION_REFRESH_EACH_REQUEST*
    ...
    if op == "set":
        ...
        if current_cookie:
            # Let's make sure it isn't expired if tsn_app doesn't set TIME_LIMIT to None.
            try:
...
def base_render_json(
    ...
    which can be overridden by the tsn_app.
    """
