...
so tsn_app must set CSRF_IGNORE_UNAUTH_ENDPOINTS (or use CSRF/session cookie for logging
...
def register() -> ResponseValue:
    ...
    if form.validate_on_submit():
        ...
        # Only include security token if in fact user is permitted to login
...
def two_factor_token_validation():
    ...
    if changing:
        if _security._want_json(request):
            return base_render_json(form)
        # allow tsn_app to fetch just this form (independent of /tf_setup)
        return _security.render_template(
...
def two_factor_rescue():
    ...
    if form.validate_on_submit():
        ...
        # send tsn_app provider a mail message regarding trouble
        elif raction == "help":
            send_mail(
