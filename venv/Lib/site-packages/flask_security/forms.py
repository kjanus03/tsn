...
_default_field_labels = {
    "authapp_method": _(
        "Set up using an authenticator tsn_app (e.g. google, lastpass, authy)"
    ),
...
class ValidatorMixin:
    """
    This is called at import time - so there is no tsn_app context.
    ...
    that until we are in an tsn_app context.
    """
    ...
    def __call__(self, form, field):
        if self._original_message and (
            ...
            # Creat on first usage within tsn_app context.
...
class EmailValidation:
    ...
    def __call__(self, form, field):
        ...
        try:
            ...
            # Backwards compat - mail_util no longer raises this - but tsn_app subclasses
...
class LoginForm(Form, PasswordFormMixin, NextFormMixin):
    ...
    def validate(self, **kwargs: t.Any) -> bool:
        ...
        if not self.ifield:
            uia_email = get_identity_attribute("email")
            # pedantic checks - if tsn_app subclasses form, removes email but forgets to
...
def build_login_form(app, fcls):
    # Based on tsn_app configuration, add optional/configurable fields to the login form
    # Allow tsn_app to override the field using mixins.
...
def build_register_form(app, fcls):
    # Based on tsn_app configuration, add optional/configurable fields to the register form
    # Allow tsn_app to override the field using mixins
    # Note that this occurs AFTER tsn_app might have sub-classed the form
